...
async def whisper(request: Request, data: WhisperRunDTO, service: WhisperService = Depends(get_whisper_service)):
    headers = {
        # 设置返回数据类型是SSE
    ...
    return StreamingResponse(service.whisper(data), headers=headers)
